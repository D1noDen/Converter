...
import { Space, InputNumber , Button } from "antd"
 const ConvertVh = () => {
    const [sizeVW , SetSizeVW] = useState<number>(1080)
...
<InputNumber size="large" type='number' min={1} max={100000} defaultValue={1080} style={{width:'20vw'}}   onChange={(e:number|any) => SetSizeVW(e)} />
</div>
<div style={{display:'flex' , flexDirection:"column"}}>
<label>Widht</label>
<InputNumber size="large" type='number' min={1} max={100000} style={{width:'20vw'}} onChange={(e:number|any) => SetWidthVW(e)} />
</div>
<div className="Convertor__Result">{resultSize.toFixed(2) + ' vh'}</div>
</div>
